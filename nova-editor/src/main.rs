use nova::{DefaultPlugins, core::{EngineResult, app::{ApplicationBuilder, ApplicationProxy}}, egui};
...
impl ApplicationProxy for EditorApp {
    fn on_init(&mut self, _ctx: &mut nova::core::app::ApplicationContext) -> nova::core::EngineResult<()> {
        Ok(())
    }

    fn on_update(&mut self, _ctx: &mut nova::core::app::ApplicationContext, _dt: std::time::Duration) {
    }

    fn on_render(&mut self, _ctx: &nova::core::app::ApplicationContext, _frame: &mut nova::core::graphics::frame::Frame) {
    }

    fn on_gui(&mut self, ui: &mut nova::egui::Ui) {
        self.tabs.ui(ui);
    }
}
